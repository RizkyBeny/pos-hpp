...
import { LucideCalendar, LucideClock, LucideFileSignature, LucideTrash2, LucideCheckCircle2, LucideCopy } from 'lucide-react';
...
export default function ManualEntryForm({ isDemoMode = false }: ManualEntryFormProps) {
    ...
    const [notes, setNotes] = useState('Rekap Manual Transaksi');
    ...
    const handleSaveEntry = async () => {
        ...
        try {
            if (isDemoMode) {
                setTimeout(() => {
                    setSuccessMessage(`Catatan manual berhasil disimpan (Demo).`);
                    clearCart();
                    setNotes('Rekap Manual Transaksi');
            ...
            setSuccessMessage(`Catatan manual tanggal ${saleDate} berhasil disimpan.`);
            clearCart();
            setNotes('Rekap Manual Transaksi');
            setTimeout(() => setSuccessMessage(null), 3000);
        } catch (error) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                    <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
                        <div className="flex flex-col space-y-1.5 p-6 border-b border-orange-100">
                            <h3 className="font-semibold leading-none tracking-tight flex items-center gap-2">
                                <LucideFileSignature className="h-5 w-5 text-orange-500" />
                                Entri Transaksi Manual
                            </h3>
                            <p className="text-sm text-muted-foreground">Catat transaksi yang sudah terjadi di masa lalu atau rekap massal.</p>
                        </div>
                        <div className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className="text-sm font-medium leading-none">Tanggal Transaksi</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-sm font-medium leading-none">Waktu Transaksi</label>
                                    <div className="relative">
                                        <LucideClock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                                        <input
                                            type="time"
                                            className="flex h-10 w-full rounded-md border border-input bg-background pl-10 pr-3 py-2 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                                            value={saleTime}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium leading-none">Keterangan Tambahan</label>
                                <textarea
                                    ...
                                    placeholder="Rekap penjualan pameran siang..."
                                    value={notes}
                            ...
                            <button
                                type="button"
                                className="w-full flex items-center justify-center gap-2 h-10 rounded-lg border-2 border-dashed text-zinc-500 hover:text-orange-500 hover:border-orange-500 transition-colors bg-zinc-50 dark:bg-zinc-900/50"
                            >
                                <LucideCopy className="h-4 w-4" />
                                <span className="text-sm font-bold">Duplikat Transaksi Kemarin (Auto-fill)</span>
                            </button>
                        </div>
                    </div>
                </div>

                {/* Cart Section Specific to Manual Entry */}
                <div className="space-y-6">
                    <div className="rounded-xl border bg-card text-card-foreground shadow-sm">
                        <div className="flex flex-col space-y-1.5 p-6 border-b">
                            <h3 className="font-semibold leading-none tracking-tight">Rincian Produk Terjual</h3>
                            <p className="text-sm text-muted-foreground">Pilih produk dan jumlah dari tab "Cari Menu".</p>
                        </div>
                        <div className="p-6">
                            {cart.length === 0 ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="pt-4 border-t space-y-3">
                                        <div className="flex justify-between items-center text-lg font-black text-orange-600 dark:text-orange-500">
                                    ...
                                    <div className="pt-4">
                                        <button
                                            ...
                                            className="w-full flex items-center justify-center h-12 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            {isSubmitting ? 'Menyimpan...' : 'Simpan Entri Manual'}
                                        </button>
                                    </div>
                                </div>
                            )}
